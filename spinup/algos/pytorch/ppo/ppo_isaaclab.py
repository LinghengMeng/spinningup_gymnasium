...
run = wandb.init(project="spinup_ppo", name="{}_{}".format(args_cli.task, datetime.datetime.now().strftime('%Y-%m-%d_%H-%M-%S,')), config=config)
###################################################
...
def ppo(env_fn, actor_critic=core.MLPActorCritic, ac_kwargs=dict(), seed=0, 
    ...
    action_space = Box(env.action_space.low[0][0], env.action_space.high[0][0], shape=(env.action_space.shape[1],))

    obs_dim = observation_space.shape                                  # Adapt to IsaacLab
    act_dim = action_space.shape                                       # Adapt to IsaacLab
    ...
    for epoch in range(epochs):
        for t in range(local_steps_per_epoch):
            ...
            next_o = next_o['policy'].squeeze(0).cpu()  # Adapt to IsaacLab
...
if __name__ == '__main__':
    ...
    ppo(env, actor_critic=core.MLPActorCritic,
        ...
        clip_ratio = wandb.config.clip_ratio,
        pi_lr = wandb.config.pi_lr,
        vf_lr = wandb.config.vf_lr,
        train_pi_iters = wandb.config.train_pi_iters,
