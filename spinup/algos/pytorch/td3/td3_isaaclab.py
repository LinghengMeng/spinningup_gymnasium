...
run = wandb.init(project="spinup_td3", name="{}_{}".format(args_cli.task, datetime.datetime.now().strftime('%Y-%m-%d_%H-%M-%S,')), config=config)
###################################################
...
def td3(env_fn, actor_critic=core.MLPActorCritic, ac_kwargs=dict(), seed=0, 
        ...
        update_after=1000, update_every=50, act_noise=0.1, target_noise=0.2, 
        noise_clip=0.5, policy_delay=2, num_test_episodes=10, act_limit=None, max_ep_len=1000, 
    ...
    ac = actor_critic(observation_space, action_space, **ac_kwargs)  # Adapt to IsaacLab
    ac_targ = deepcopy(ac)
    ...
    def update(data, timer):
        ...
        wandb.log({"LossQ":loss_q.item(), **loss_info})
...
if __name__ == '__main__':
    ...
    td3(env, actor_critic=core.MLPActorCritic,
        ...
        pi_lr = wandb.config.pi_lr,
        q_lr = wandb.config.q_lr,
        batch_size = wandb.config.batch_size,
        start_steps = wandb.config.start_steps,
        update_after = wandb.config.update_after,
        update_every = wandb.config.update_every,
        act_noise = wandb.config.act_noise,
        target_noise = wandb.config.target_noise,
        noise_clip = wandb.config.noise_clip,
